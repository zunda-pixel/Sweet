...
public struct TwitterOAuth1 {
  ...
  private func authorizationToken(bearerToken: String) async throws -> String {
    ...
    let values = stringData.split(separator: "&").reduce(into: [String: String]()) { partialResult, next in
  ...
  static public func accessToken(oAuthToken: String, oAuthVerifier: String) async throws -> OAuth1Response {
    let parameters = [
    ...
    let response = OAuth1Response(oAuthToken: values["oauth_token"]!,
