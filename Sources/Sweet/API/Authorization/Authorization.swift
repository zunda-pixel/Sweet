...
extension Sweet {
  ...
  static public func getAppBearerToken(apiKey: String, apiSecretKey: String, session: URLSession = .shared) async throws -> String {
    ...
    let (data, _) = try await session.post(url: url, headers: headers, queries: queries)
